...
const (
	aliAppid                    = "${aliAppid}"
	alipayCertDir               = "doc/alipay/"
	alipayPrivateKeyFilePath    = alipayCertDir + "privateKey.pem"
	alipayAppPublicCertFilePath = alipayCertDir + "appPublicCert.crt"
	alipayRootCertFilePath      = alipayCertDir + "alipayRootCert.crt"
	alipayPublicCertFilePath    = alipayCertDir + "alipayPublicCert.crt"
)
...
const (
	...
	paymentDate      string  = "gmt_payment"
	tradeStatus      string  = "trade_status"
...
func init() {
	...
	PayMap[AliPay] = &payAction{
		...
		PayNotify: func(request *http.Request, data string) (paymentUpdateInput PaymentUpdateI, err error) {
			u, err := url.ParseQuery(data)
			if err != nil {
				return
			}

			ok, err := aliClient.VerifySign(u)
			if err != nil {
			...
			if ok == false {
				err = fmt.Errorf("【支付宝】异步通知验证签名未通过")
				return
			}

			outTradeNo := u.Get(outTradeNoField)
			paymentDate := u.Get(paymentDate)
			tradeStatus := u.Get(tradeStatus)
		...
		statusQuery: func(client any, ctx context.Context, outTradeNo string) (res any, err error) {
			...
			if resp.IsSuccess() == false {
				err = fmt.Errorf(resp.Content.SubMsg)
				return
			}
			res = resp
			return
